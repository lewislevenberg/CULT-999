  ...
  /*fix bug for in french translate english labels*/
    ...
    if ($('.oecd-main-nav .list-wrapper .sub-list-wrapper').length == 0) {
        ...
        countrywrapper.find('>li').each(function() {
            ...
            for (var i in tabs) {
                if (letter >= tabs[i].type.charAt(0) && letter <= tabs[i].type.charAt(2)) {
                    ...
                    if (hasLink) {
                        tabs[i].items.push('<li data-sort-char="' + firstTwoLetter + '">' + $(this).html() + '</li>');
                    }
                    $(this).remove();
                }
            }
        ...
        for (var i in tabs) {
            ...
            countrywrapper.append("<li><a href='/'>" + tabs[i].type + "</a><ul class='dropdown-sub-tab'></ul></li>");
            countrywrapper.find('>li:eq(' + i + ')>ul').append(tabs[i].items);
        ...
        countrywrapper.each(function() {
            $(this).navList({
                columns: 6,
                subclassname: 'dropdown-tab',
                icon: false,
                slice: false
            });
        });
        countrywrapper.find('.dropdown-sub-tab').each(function() {
            $(this).navList({
                columns: 5,
                subclassname: 'sub-sub-tab',
                icon: true,
                slice: true
            });
        });

        countrywrapper.find('.dropdown-tab').mouseenter(function() {
            countrywrapper.find('.dropdown-tab').find(".dropdown-sub-tab").hide();
            $(this).find(".dropdown-sub-tab").show();
        });
        countrywrapper.find('.dropdown-tab').mouseenter();
