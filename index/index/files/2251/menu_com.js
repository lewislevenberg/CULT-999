...
function ClcJus(){
	var x=RcrsLvl==1?BorderWidth:BorderSubWidth;
	var a=BorderBtwnElmnts?1:2,b=BorderBtwnElmnts?x:0;
	var Size=Math.round(((FrstWinWdth-a*x)/NoOffFirstLineMenus)-b),i,j;
...
function PosMenu(CntnrPntr,Tp,Lt){
	...
	if (RcrsLvl==1 && FirstLineHorizontal){
		Hori=1;Lefti=CntnrWt-MmbrWt-2*x;Topi=0}
	else{	Hori=Lefti=0;Topi=CntnrHt-MmbrHt-2*x}
	while(Mmbr!=null){
		MmbrStyle.left=Lefti+x;
		MmbrStyle.top=Topi+x;
		if(Nav4)Mmbr.CmdLyr.moveTo(Lefti+x,Topi+x);
		if(Mmbr.ChildCntnr){
			if(RightToLeft)ChldCntnrWdth=Nav4?Mmbr.ChildCntnr.clip.width:parseInt(Mmbr.ChildCntnr.style.width);
			if(Hori){	SubTp=Topi+MmbrHt+2*x;
				SubLt=RightToLeft?Lefti+MmbrWt-ChldCntnrWdth:Lefti}
			else{	SubLt=RightToLeft?Lefti-ChldCntnrWdth+ChildOverlap*MmbrWt+x:Lefti+(1-ChildOverlap)*MmbrWt+x;
		...
		if(Mmbr){	MmbrStyle=!Nav4?Mmbr.style:Mmbr;
			...
			Hori?Lefti-=BorderBtwnElmnts?(MmbrWt+x):(MmbrWt):Topi-=BorderBtwnElmnts?(MmbrHt+x):(MmbrHt)}}
	RcrsLvl--}
...
function CreateMenuStructure(MName,NumberOf){
	...
	if (RcrsLvl==1&&FirstLineHorizontal){
		...
		Wdth=BorderBtwnElmnts?Wdth+(NumberOf+1)*x:Wdth+2*x;Hght=MenuHeight+2*x}
	else{	for(i=1;i<NumberOf+1;i++){
			WMnu=MName+eval(i);
			Hght=eval(WMnu+'[4]')?Hght+eval(WMnu+'[4]'):Hght+MenuHeight}
		Hght=BorderBtwnElmnts?Hght+(NumberOf+1)*x:Hght+2*x;Wdth=MenuWidth+2*x}
	if(DomYes){
