...
Customized for IMF.org by ICF Interactive - 4/29/2014*/
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,swf,png,csv,jpg,jpeg,gif,docx,xlsx,xlsm,ashx"
s.linkInternalFilters="javascript:,imf.org,www.imf.org,www.internationalmonetaryfund.com,www.internationalmonetaryfund.org,http://adlib.imf.org/digital_assets/,http://www.imf.org/EXTERNAL/ADLIB_IS4/findingaids.aspx"
...
/* Replaces the getPageName Plug-in and assigns pagename using the same syntax */
if(!s.pageType && !s.pageName){
	s.pageName=location.pathname.toLowerCase() + location.search.toLowerCase() + document.title;
	s.prop8=window.location.search;
	s.prop9=document.title;
	};
	
	/* Remove leading/trailing colons in the pageName */		
	if (s.pageName.substr(0,1) == "/") {
		s.pageName = s.pageName.substring(1);
		};
	var len = s.pageName.length;
	if (s.pageName.substr(len-1,1) == "/") {
		s.pageName = s.pageName.substring(0,len-1);
		};
...
s.doPlugins = function(s) {
	...
	s.prop4=s.Util.getQueryParam('NewQuery');
	...
	if(s.prop4){
		...
		s.prop6=s.Util.getQueryParam('lan');
		s.prop7=s.Util.getQueryParam('col');
		};
 ...
 * NOTE - Not tested for AppMeasurement.js, but should be supported.  Validate in DEV
 */
